...
namespace HexGame {
	...
	template <template<class ...> class T>
	HexAdapter<T>::HexAdapter(int board_size) {
		size = board_size;
		choice = 0;
		turn = 0;
		int y, x;
	...
	template <template<class ...> class T>
	void HexAdapter<T>::resize_board() {
		int i, y, x;
	...
	template <template<class ...> class T>
	bool HexAdapter<T>::readFromFile(const string filename) {
		bool success = 0;
		int old_size;
		fstream file;
	...
	template <template<class ...> class T>
	int HexAdapter<T>::play(Cell_state user_mark) {
	...
	template <template<class ...> class T>
	Cell_state HexAdapter<T>::operator()(const int coor_y, const int coor_x) const{
	...
	template class HexAdapter<vector>;
	template class HexAdapter<deque>;
